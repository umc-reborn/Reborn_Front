...
class NewShopService {
    ...
    static let shared = NewShopService()
    private init() {}
    
    func getNewShop(completion: @escaping (NetworkResult<Any>) -> Void) {
        let url: String! = APIConstants.newStoreURL
...
//                     dump(value)
                     let networkResult = self.judgeStatus(by: statusCode, value, NewShopModel.self)
